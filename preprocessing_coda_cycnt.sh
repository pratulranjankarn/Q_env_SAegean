...
rm -f ~/env_EGEL/intgrl_cycnt_compil_coda_nr.txt
...
samp_st = round(2*sdist/v_avg*fsamp + 1*fsamp);
samp_end = round(sdist/v_avg*fsamp + 46*fsamp);
...
EOF
#SOD
cat env_integral_coda.txt >> /home/user/env_EGEL/intgrl_cycnt_compil_coda_nr.txt
cd ..
done
