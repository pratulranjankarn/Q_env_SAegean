...
for h in /home/user/env_EGEL/kef_events/2* #/home/user/env_EGEL/cornoth_events/2*
#/home/user/env_EGEL/husn_events/2012-01-{01..17..1}*
#/home/user/env_EGEL/husn_events/2010* /home/user/env_EGEL/husn_events/2011-{01..07..1}* 
do
	echo "${h}"
	#sudo swapoff -a
	#sudo swapon -a	
	...
	rm -f ${h}/*.sacii
	rm -f ${h}/*.ascii
	aw=`find ${h}/ -name "husn.2*.hyp"`
	if [ "${#aw}" -gt 0 ]; then
		err_xx=`grep "STATISTICS" ${h}/husn.2*.hyp | awk '{ print $9 }'`
		err_yy=`grep "STATISTICS" ${h}/husn.2*.hyp | awk '{ print $15 }'`
		err_zz=`grep "STATISTICS" ${h}/husn.2*.hyp | awk '{ print $19 }'`
		...
		if (( $(echo "${err_h} < 5" | bc) )) && (( $(echo "${err_zz} < 5" | bc) )); then
			...
			do
				...
				if [[ "${#i}" -gt 64 ]]; then
					...
					if [ "${chek_sz}" -gt 70000 ] && [ "${chek_sz}" -lt 140000 ]; then
						echo $i
						IFS="/"
						read -ra adr <<< "${i}"
						IFS="."
						...
						echo $stnm $comp
						IFS=${OLDIFS}
						aw=`grep -w "$stnm" ${h}/husn.2*.hyp | grep " S"`
						...
						if [[ "${#aw}" -gt 0 ]]; then
							...
							pzf=`find /home/user/env_EGEL/HUSN_PZs/* -name "SAC*${stnm}_${comp}*"`
							#echo $pzf 
							j=0
							for j in ${pzf};
							do
