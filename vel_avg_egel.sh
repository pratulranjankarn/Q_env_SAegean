...
do
echo ${i:16:4}
aw=`find ./EGEL_events/ -name "egel.20*.hyp" -exec grep "${i:16:4} " {} + | grep "? S      ?"`
echo ${#aw}
if [[ "${#aw}" -gt 0 ]]; then
find ./EGEL_events/ -name "egel.20*.hyp" -exec grep "${i:16:4} " {} + | grep "? S      ?" > temp2.txt
mw=`grep "${i:16:4}" ./stlist_egel.txt`
qw=`grep "${i:16:4}" ./stlist_husn.txt`
if [[ "${#mw}" -gt 0 ]] && [[ "${#qw}" -eq 0 ]]; then
...
do
Odep=`grep "GEOGRAPHIC" ${V1:0:68} | awk '{ print $14 }'`
epdist=${V22}
distnce=$(echo "sqrt(${Odep}^2 + ${epdist}^2)" | bc -l)
if (( $(echo "${distnce} <= 100" | bc -l) )); then
echo ${V1:0:68}
Ohrmin=`grep "GEOGRAPHIC" ${V1:0:68} | awk '{ print $6$7 }'`
Osec=`grep "GEOGRAPHIC" ${V1:0:68} | awk '{ print $8 }'`
echo $Ohrmin $Osec
Shrmin=${V8}
Ssec=${V9}
echo $Shrmin $Ssec
