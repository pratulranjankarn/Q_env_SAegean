...
rm -f ~/env_EGEL/intgrl_egel_Stest.txt
...
list = dir('out_N_PE01*envwin*.txt');

filnam = sprintf('env_integral_test.txt');
fid = fopen(filnam,'w');
...
U4 = trapz(dta(fsamp*61:fsamp*71))/(fsamp*10);
...
EOF
#SOD
cat env_integral_test.txt >> /home/user/env_EGEL/intgrl_egel_Stest.txt
rm -f env_integral_test.txt
cd ../..
done < Stest.txt
