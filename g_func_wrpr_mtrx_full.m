function error_sum = g_func_wrpr_mtrx_full(g_st,E,tm,v_avg,r,sz)
...
idx_int = zeros(length(g_st),1,'single');

for i=1:length(g_st)
   [Ao,~] = intersections(tm',E_calc(:,i),tm',E);
   if (length(Ao) >= 2)
    idx_int(i) = single(Ao(2));
   end
end

w_len = round(median(idx_int)*100);

alpha1 = log(mean(exp(lG_f1(1:w_len+1,1:n_g))));
alpha2 = lG_f1((w_len+2):sz,1:n_g); G_f = cat(1,alpha1,alpha2);

S_idx = single(find(E==max(E)));

tm = [single((S_idx(1)-101)/100) ...
                        single(linspace((w_len/100+0.01),t_len,...
                         sz-w_len-1))];
...
E2(1) = mean(E1(1:w_len+1));
E2(2:(sz-w_len)) = E1((w_len+2):end);
E2 = log(E2);
