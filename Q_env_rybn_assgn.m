%% Assign values to bins traversed by ray path
%%{
...
for i=1:length(A{1})
   ...
   b = track(waypts(:,1),waypts(:,2),referenceEllipsoid,'degrees',1000);
   b(1001,:) = [];
   [trk_x,trk_y] = geodetic2ecef(wgs84,b(:,1),b(:,2),15);
   dst_x = arrayfun(@(x)(Nx-x).^2,trk_x,'UniformOutput',false);
   dst_y = arrayfun(@(y)(Ny-y).^2,trk_y,'UniformOutput',false);
   dst_x = dst_x'; dst_y = dst_y'; dst = cell2mat(dst_x) + cell2mat(dst_y);
   [~,dst_min] = mink(dst,1);
   id = unique(dst_min);
   
   n_arr15{i} = [uint32(id') uint32(ones(length(id),1)*A{8}(i)*v_avg*factr)...
       uint32(ones(length(id),1)*A{9}(i)*factr) uint32(ones(length(id),1)*v_avg*factr)];
   else
...
g_f1 = zeros(length(un_12),1,'single');
b_f1 = zeros(length(un_12),1,'single');
...
Qsc_inv = g_f1/(2*pi*freq);
Qi_inv = b_f1/(2*pi*freq);
