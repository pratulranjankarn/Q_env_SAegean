...
for i=1:3%length(dr_lst)
    ...
    if (~isempty(aw))
        ...
        if (n_fls > 0)
            ...
            r = zeros(n_fls,1,'single');
            v_avg = zeros(n_fls,1,'single');
            S_idx = zeros(n_fls,1,'single');
            sz = zeros(n_fls,1,'single');
            t_end = zeros(n_fls,1,'single');
            g_st = zeros(n_fls,1,'single');
            b_vl = zeros(n_fls,1,'single');
            tm_cl = cell(1,n_fls);
            stnm = cell(1,n_fls);
            ...
            for j=1:n_fls
                ...
                sum_tmp = sum_tmp + sum(abs((Earset_cl{j}-E_cl{j})))/length(Earset_cl{j})
                err_tmp = err_tmp + abs((E_cl{j}(1) - Earset_cl{j}(1)))
                error_prom = error_prom + abs(err_tmp/Earset_cl{j}(1));
                
                plot(E_cl{j}); hold on; plot(Earset_cl{j}); hold off;
            end
            %{
