...
#   echo ${dte[2]}
    ...
    cornoth_pick_fnl.txt | grep "T__" > test.txt
    ...
    d_o=${dte[2]}
        ...
        if [ "${pick_arr[3]:0:1}" == "P" ] || [ "${pick_arr[3]:0:1}" == "S" ]; then
            ...
            echo ${stnm} ? ${comp} ? ${phz} ? ${dyt_fnl} ${hrm_fnl} ${ss_fnl} GAU ${ph_err}e+00 -1.00e+00 -1.00e+00 -1.00e+00 >> cornoth_shlw.pick
        fi
    done < test.txt
    echo " " >> cornoth_shlw.pick
done < cornoth_evinfo_fnl.txt